...
public interface MenuRepository extends JpaRepository<Menu, Long> {
	...
	@Query(value="select m.id ,m.name,m.parent_id from menu m where m.parent_id=0",nativeQuery = true)
	List<Menu> readAllbyParent();
	@Query(value="select m.id ,m.name,m.parent_id from menu m where m.parent_id=:menuId",nativeQuery = true)
